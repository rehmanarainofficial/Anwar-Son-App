...
const CRMWorkshopRequestScreen = ({ navigation, route }) => {
  ...
  const loadWorkshopData = useCallback(async () => {
    ...
    try {
      ...
      const res = await getWorkshopData(payload).unwrap();
  ...
  }, [user?.id, user?.role_id, fromDate, toDate, getWorkshopData]);
